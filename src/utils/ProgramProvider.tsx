...
export const ProgramProvider: FC<ProgramProviderProps> = ({ children }) => {
  ...
  const getABHbalance = async () => {
    try {
      let tokenContract = new web3.web3.eth.Contract(
        ERC20_ABI,
  ...
  const getStakingPoolData = async () => {
    try {
      let StakingContract = new web3.web3.eth.Contract(
        STAKING_ABI,
        InfoStaking.contract
      );
  ...
  const stake_token = useCallback(
    async (_amount: number) => {
      ...
      let amount = _amount * 10 ** InfoCoinflip.token_decimals;

      let balance = await tokenContract.methods.balanceOf(wallet).call();
      if (balance < amount) throw new Error('Not Enough Token');

      let allowance = await tokenContract.methods
        .allowance(wallet, InfoStaking.contract)
        .call();
      if (allowance < amount) {
        await tokenContract.methods
          .approve(
            InfoStaking.contract,
            '0x' + (amount - allowance).toString(16)
          )
          .send({
            from: wallet,
            gas: 300000,
  ...
  const coinflip_flip = useCallback(
    async (selectedSide: boolean, selectedAmount: number) => {
      ...
      let coinFlipContract = new web3.web3.eth.Contract(
        FLIP_ABI,
        InfoCoinflip.contract
      );
      ...
      let allowance = await tokenContract.methods
        .allowance(wallet, InfoCoinflip.contract)
        .call();
      if (allowance < amount) {
        await tokenContract.methods
          .approve(
  ...
  const coinflip_claim = useCallback(async () => {
    ...
    let pendingAmount = await coinFlipContract.methods
      .pendingAmount(wallet)
      .call();
  ...
  const dice_roll = useCallback(
    async (selectedCase: number, selectedAmount: number) => {
      ...
      let diceContract = new web3.web3.eth.Contract(
        DICE_ABI,
        InfoDice.contract
      );
      ...
      let allowance = await tokenContract.methods
        .allowance(wallet, InfoDice.contract)
        .call();
      if (allowance < amount) {
        await tokenContract.methods
      ...
      let result = await diceContract.methods
        .flip(selectedCase, '0x' + amount.toString(16))
        .send({
          from: wallet,
          gas: 300000,
          gasPrice: 5000000000
